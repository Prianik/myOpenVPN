...
function installUnbound() {
	...
	if [[ ! -e /etc/unbound/unbound.conf ]]; then
		...
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			# Configuration
			echo 'interface: ${IPVPN}.1
access-control: ${IPVPN}.1/24 allow
hide-identity: yes
hide-version: yes
use-caps-for-id: yes
prefetch: yes' >>/etc/unbound/unbound.conf
			...
			# Configuration
			sed -i 's|# interface: 0.0.0.0$|interface: ${IPVPN}.1|' /etc/unbound/unbound.conf
			sed -i 's|# access-control: 127.0.0.0/8 allow|access-control: ${IPVPN}.1/24 allow|' /etc/unbound/unbound.conf
...
#-orig		SERVER_CN="cn_$(head /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1)"
		SERVER_CN="cn_$(head /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1)"
		echo "$SERVER_CN" >SERVER_CN_GENERATED
#-orig		SERVER_NAME="server_$(head /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1)"
		SERVER_NAME="server"
...
if [ ! -d "/etc/openvpn/_OpenVPN_KEY" ]; then
	mkdir /etc/openvpn/_OpenVPN_KEY
fi

cp $homeDir/$CLIENT.ovpn  /etc/openvpn/_OpenVPN_KEY/
...
#-----
rm -f /etc/openvpn/_OpenVPN_KEY/$CLIENT.ovpn 
